...
def get_common_word_tokens(
    ...
    return get_tokens(tokenizer, filter_common_words, replace_special_chars=replace_special_chars)
...
def get_common_words(threshold=10) -> dict[str, int]:
    ...
    return { word: word_freq[word.lower()] for word in all_words if word_freq[word.lower()] >= threshold }


def group_words_by_ending_and_length(words: list[str]) -> dict[int, dict[str, list[str]]]:
    """
    ...
    return {
        length: {ending: words for ending, words in length_group.items() if len(words) > 1}
        for length, length_group in groups.items()
    }


def get_similar_word_pairs(length: int) -> dict[str, str]:
    """
    ...
    for similar_words in word_groups[length].values():
      pairs[similar_words[0]] = similar_words[1]
